...
; ********************************************************
; CS 201 HW #3  DUE Monday, 10/11/2021 at 11:59 pm
...
(define (i-lookup state symbol mach)
  (cond [(null? mach) #f]
        [(i-match? state symbol (first mach)) (first mach)]
        [else (i-lookup state symbol (rest mach))]
    ))
...
(define (halted? mach config)
  (if (equal? (i-lookup (conf-state config) (conf-symbol config) mach) #f)
      #t
      #f
      )
  )
...
(define (b-remove-first lst)
  (cond [(null? lst) '()]
        [(equal? (first lst) 'b) (b-remove-first (rest lst))]
        [else lst]
        )
  )
...
(define (firss lst)
  (cond [(null? lst) '()]
        [(equal? (length lst) 1) '()]
        [else (append (list (car lst)) (firss (cdr lst)))]
    )
  )

(define (b-remove-last lst)
  (cond [(null? lst) '()]
        [(equal? (last lst) 'b) (b-remove-last (firss lst))]
        [else lst]
        )
  )

(define (normalize config)
  (conf (conf-state config) (b-remove-first (conf-ltape config)) (conf-symbol config) (b-remove-last (conf-rtape config)))
  )
...
(define (shift-head-left config)
  (define left (if (empty? (conf-ltape config)) '(b) (conf-ltape config)))
  
  (normalize (conf (conf-state config) (drop-right left 1) (last left) (append (list (conf-symbol config)) (conf-rtape config))))
  )

(define (shift-head-right config)
  (define right (if (empty? (conf-rtape config)) '(b) (conf-rtape config)))
  (normalize (conf (conf-state config) (append (conf-ltape config) (list (conf-symbol config))) (first right) (rest right)))
  )
...
(define (next-config mach config)
  (define instr (i-lookup (conf-state config) (conf-symbol config) mach))

  (if (equal? instr #f)
      config
      (if (equal? (ins-dir instr) 'L)
          (shift-head-left (write-symbol (ins-n-symbol instr)(change-state (ins-n-state instr) config)))
          (shift-head-right (write-symbol (ins-n-symbol instr)(change-state (ins-n-state instr) config)))
          )
...
(define tm-convert
  ...
  (ins 'q1 1 'q1 1 'R)
  (ins 'q1 0 'q1 0 'R)
  (ins 'q1 'b 'subtract 'b 'L)
  (ins 'q1 'x 'subtract 'x 'L)

  (ins 'subtract 1 'addx 0 'R)
  (ins 'subtract 0 'subtract 1 'L)
  (ins 'subtract 'b 'cleanup 'b 'R)

  (ins 'cleanup 1 'cleanup 'b 'R)
  
  (ins 'addx 'x 'addx 'x 'R)
  (ins 'addx 0 'addx 0 'R)
  (ins 'addx 1 'addx 1 'R)
  (ins 'addx 'b 'startlist 'x 'L)

  ;;moves to beginning of list
  (ins 'startlist 'x 'startlist 'x 'L)
  (ins 'startlist 0 'startlist 0 'L)
  (ins 'startlist 1 'startlist 1 'L)
  (ins 'startlist 'b 'q1 'b 'R)
  )
  )
...
(define tm-sort
  ...
  (ins 'nZero 1 'nZero 1 'R)
  (ins 'nZero 0 'swapOne 1 'L)
  (ins 'nZero 'b 'end 'b 'L)
  
  (ins 'swapOne 1 'startlist 0 'L)
