...
; Extension to Wednesday December 1, 11:59pm.
;************************************************************
; Name: Muyi Aghedo
...
(define (ok-string? value)
  (cond [(not (list? value)) #f]
        [(empty? value) #t]
        [(not (symbol? (first value))) #f]
        [else (ok-string? (rest value))]
        )
)

(define (reg-exp? value)
  (cond [(ok-string? value) #t]
...
(define (pick lst)
  (if (empty? lst) #f (list-ref lst (random 0 (length lst))))
)
...
(define (generate-string-from-reg-exp exp)
  (cond [(ok-string? exp) exp]
        ...
        [(repeat? exp) (re (repeat-arg1 exp))]
   )
  )

(define (re exp)
  (cond [(flip 0.5) '()]
        [else (append (generate-string-from-reg-exp exp) (re exp))]
        )
  )
...
(define (lookup state symbol table)
  (cond [(null? table) #f]
        [(equal? (list state symbol) (entry-key (first table))) (entry-value (first table))]
        [else (lookup state symbol (rest table))]
        )
  )

(define (dfa-accepts? mach str)

  (define strt (dfa-start-state mach))
  (define translist (dfa-transitions mach))
  (define final (dfa-accepting-states mach))
  
  (stringCheck translist str strt final)
  )

(define (stringCheck translist str curState final)
  (cond [(null? str) (ormap (lambda (x) (equal? x curState)) final)]
        [else (let ([nState (lookup curState (first str) translist)]) (if (equal? nState #f) #f (stringCheck translist (rest str) nState final))) ]
        )
)
...
(define (list-leaf-labels tree)
  (cond [(leaf? tree) (list (leaf-label tree))]
        [else (flatten (build-list (length (node-children tree)) (lambda (x) (list-leaf-labels (list-ref (node-children tree) x))) ))]
    )
)
...
(define (generate-parse-tree-from-cfg grammar)
  (define rules (cfg-rules grammar))
  (define start (cfg-start-symbol grammar))
  (define terminus (cfg-terminals grammar))
  
  (travelDown start rules terminus)
)

(define (travelDown currentSymbol rules terminus)
  (cond [(inList? currentSymbol terminus) (leaf currentSymbol)]
        [else (let ([nextRule (chooseRandomRule currentSymbol rules)]) (node currentSymbol (map (lambda (x)  (travelDown x rules terminus)) (rule-rhs nextRule))) )]

        )
)

(define (chooseRandomRule symbol rules)
  (define validRules (filter (lambda (x) (equal? (rule-lhs x) symbol)) rules))
  (pick validRules)
  )

(define (inList? value lst) (ormap (lambda (x) (equal? x value)) lst))
...
(define dfa-mcd
  (dfa
   ...
   (list
    ...
    (entry '(vp chased) 'np2)
    (entry '(vp evaded) 'np2)
    (entry '(vp believed) 'vvs)
    (entry '(vp dreamed) 'vvs)
    (entry '(vvs that) 'snp)
    (entry '(np2 it) 'acc)
    (entry '(np2 a) 'n2)
    (entry '(np2 the) 'n2)
...
(define exp-standard-pred
  ;;(slept|swam)|((chased|evaded) exp-subject)
  (either (either '(slept) '(swam)) (concat (either '(chased) '(evaded)) exp-subject))
  )

(define exp-nonstandard-pred
  ;;((believed|dreamed) that)
  (concat (either '(believed) '(dreamed)) '(that))
  )

(define exp-recursive-aspect
  (concat exp-subject exp-nonstandard-pred))

(define exp-mcd
  (concat exp-subject (either exp-standard-pred (concat (concat exp-nonstandard-pred (repeat exp-recursive-aspect)) (concat exp-subject exp-standard-pred))))
  )
...
(define my-cfg
  (cfg
   '(Bobby Karol Tyler walked went a the dog cat to see find school park landmark market who)
   '(stt propN pred art simpN prepos loop locat sv)
   'stt
   (list
    (rule 'stt '(propN pred))
    (rule 'stt2 '(propN who pred))
    (rule 'propN '(Bobby))
    (rule 'propN '(Karol))
    (rule 'propN '(Tyler))
    (rule 'pred '(walked art simpN prepos))
    (rule 'pred '(went prepos))
    (rule 'simpN '(dog))
    (rule 'simpN '(cat))
    (rule 'prepos '(to loop))
    (rule 'prepos '(to art locat))
    (rule 'loop '(sv stt2))
    (rule 'loop '(sv propN))
    (rule 'sv '(see))
    (rule 'sv '(find))
