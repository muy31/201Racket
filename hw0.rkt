...
; ********************************************************
; CS 201 HW #0  DUE Friday 9/17/2021, 11:59 pm
...
(define (kg2lb mass)
  ;;Calculate wholes
  (define wholes (inexact->exact (floor (/ (* mass 22) 10))))
  ;;Calculate parts
  (define ounces (inexact->exact (floor (* (- (/ (* mass 22) 10) wholes) 16))))
  ;;Determine plurality of ounces
  (define endString (if (equal? ounces 1) 'ounce 'ounces))
  
  (list wholes (if (equal? wholes 1) 'pound 'pounds ) ounces endString)
  )
; ********************************************************
; ** problem 2 ** (4 points)
...
; issues: June, July, August, September 2021 See http://cacm.acm.org/
